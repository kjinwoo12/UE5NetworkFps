...
## 프로젝트 튜닝: 메시·카메라 Z 오프셋

### 적용한 오프셋 (사용자 에디터 작업)

| 컴포넌트 | Z 오프셋 | 의도 |
|----------|----------|------|
| `FirstPersonMesh` | **−20** | 몸을 아래로 — 아래 시선 시 클리핑 완화 |
| `FirstPersonCamera` | **+20** | 카메라를 위로 — **캡슐/루트 기준 시선 원점 (0,0,0) 유지** |
...
`FirstPersonMesh`가 **카메라 자식**이면 컴포넌트 Z와 리그 `BodyOffset`이 **겹쳐 적용**될 수 있습니다. PIE에서 pelvis 월드 이동을 확인하며 튜닝해야 합니다.
